(()=>{
  ...
  if('serviceWorker' in navigator&&secureContext){
    window.addEventListener('load',()=>{
      navigator.serviceWorker.register('./sw.js?v=29-private-vary-range-safe-shell',{scope:'./',updateViaCache:'none'}).then(reg=>reg.update()).catch(err=>console.warn('PWA service worker não registrado:',err));
    });
  }
