(()=>{
  ...
  if('serviceWorker' in navigator&&secureContext){
    window.addEventListener('load',()=>{
      navigator.serviceWorker.register('./sw.js?v=26-private-vary-safe',{scope:'./',updateViaCache:'none'}).then(reg=>reg.update()).catch(err=>console.warn('PWA service worker não registrado:',err));
    });
  }
